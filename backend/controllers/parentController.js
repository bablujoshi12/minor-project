...
const getChildrenList = async (req, res) => {
  try {
    ...
    const [children] = await pool.execute(
      `SELECT s.*, b.code as branch_code, b.name as branch_name, p.name as parent_name, p.email as parent_email
       FROM students s
...
const getChildAttendance = async (req, res) => {
  try {
    ...
    // Get attendance data (recent 60 days)
...
const getChildAssignments = async (req, res) => {
  try {
    ...
    const [teacherAssignments] = await pool.execute(
      `SELECT a.*, t.name as teacher_name, 'assigned' as assignment_type, NULL as is_submitted
       FROM assignments a
    ...
    const allAssignments = [
      ...teacherAssignments.map(a => ({ ...a, is_submitted: false })),
      ...submittedAssignments.map(a => ({ ...a, is_submitted: true }))
    ];
...
// Get Child Test Marks (from marks table) - Only 3rd and 4th semester
...
// Get Child Semester Results (from marks table - only 3rd and 4th semester)
const getChildSemesterResults = async (req, res) => {
  try {
    ...
    // Get marks grouped by semester - only 3rd and 4th semester
    const [results] = await pool.execute(
      `SELECT 
       ...
       FROM marks 
       WHERE student_id = ? AND semester IN (3, 4)
       GROUP BY semester
